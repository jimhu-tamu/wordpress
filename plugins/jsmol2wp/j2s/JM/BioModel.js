...
Clazz.load (["JM.Model"], "JM.BioModel", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.AU", "$.BS", "$.Lst", "$.P3", "$.PT", "$.SB", "J.api.Interface", "J.c.STR", "JM.AtomCollection", "JM.AlphaPolymer", "$.AminoPolymer", "$.Monomer", "$.Resolver", "JU.BSUtil", "$.Escape", "JV.Viewer"], function () {
...
Clazz.overrideMethod (c$, "getConformation", 
function (conformationIndex, doSet, bsSelected, bs) {
var altLocs = this.ms.getAltLocListInModel (this.modelIndex);
var nAltLocs = this.ms.getAltLocCountInModel (this.modelIndex);
if (conformationIndex > 0 && conformationIndex >= nAltLocs) return;
...
if (bsConformation.nextSetBit (0) < 0) return;
if (conformationIndex >= 0) {
if (nAltLocs > 0) for (var i = this.bioPolymerCount; --i >= 0; ) this.bioPolymers[i].getConformation (bsConformation, conformationIndex);

for (var c = nAltLocs; --c >= 0; ) if (c != conformationIndex) bsConformation.andNot (this.ms.getAtomBitsMDa (1048607, altLocs.substring (c, c + 1)));

}if (bsConformation.nextSetBit (0) >= 0) {
bs.or (bsConformation);
if (doSet) this.setConformation (bsConformation);
}}, "~N,~B,JU.BS,JU.BS");
...
var am = ms.am;
for (var i = ms.mc; --i >= 0; ) if (am[i].isBioModel && am[i].trajectoryBaseIndex == i) {
var m = am[i];
if (vHBonds == null) {
JM.BioModel.clearRasmolHydrogenBonds (ms, m, bsA);
if (!isSame) JM.BioModel.clearRasmolHydrogenBonds (ms, m, bsB);
}m.getRasmolHydrogenBonds (bsA, bsB, vHBonds, nucleicOnly, nMax, dsspIgnoreHydrogens, bsHBonds);
}
...
if (!m.isBioModel || m.trajectoryBaseIndex != m.modelIndex || (bond.order & 28672) == 0) continue;
...
bs = null;
}if (id == 0 || bsAtoms != null && needPhiPsi && (Float.isNaN (atoms[i].group.getGroupParameter (1112539145)) || Float.isNaN (atoms[i].group.getGroupParameter (1112539146)))) continue;
...
for (var i = bsCheck.nextSetBit (0); i >= 0; i = bsCheck.nextSetBit (i + 1)) if (atoms[i].checkVisible () && atoms[i].atomID == 2 && atoms[i].group.groupID != 5) vCA.addLast ((a1 = atoms[i]));
...
var struts = (m).getBioPolymer (a1.group.getBioPolymerIndexInModel ()).calculateStruts (ms, bs1, bs2, vCA, thresh, delta, strutsMultiple);
